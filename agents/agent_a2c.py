...
from nn.decoder import ObservationModel, GenericPredictorModel, QvalueModel, PolicyModel
...
class AgentA2C(object):
    def __init__(self, env, test_env, args):
        ...
        # Architecture components Control (A2C)
        self.value_model = GenericPredictorModel(args.det_size + args.stoch_size, 1, args.q_net).to(device)
        self.target_value_model = GenericPredictorModel(args.det_size + args.stoch_size, 1, args.q_net).to(device)
        self.policy_model = PolicyModel(args.det_size + args.stoch_size, self.action_space, args.policy_net).to(device)
        ...
        self.optim = torch.optim.Adam([{'params': get_parameters(self.wm_nets), 'lr': args.lr_wm},
                                       ...
                                       {'params': self.policy_model.parameters(), 'lr': args.lr_policy}])#, eps=args.epsilon)

        # Architecture components Control (A2C RH)
    ...
    def decide_action(self, latent):
        ...
        if self.args.agent == 'dqn':
            ...
            onehot_action, _ = self.policy_model(latent)
            action = torch.argmax(onehot_action).item()
    ...
    def learn(self):
        ...
        for epoch in range(self.args.n_train_epochs):
            ...
            value_loss, policy_loss = self.learn_control(latents, rewards, nonterminals)
            # Backprop
            loss = wm_loss + value_loss + policy_loss
            self._backprop(self.optim, loss, self.wm_nets + [self.value_model, self.policy_model])
    ...
    def learn_control(self, latents, rewards, nonterminals):
        # == Critic V(z_t:T-1)
        # Get predicted value distribution p(v_t|z_t)
        value_dist = self.value_model(latents[:-1])
        # Get target
        with torch.no_grad():
            # V(z_t:T)     p(v_t|z_t) this will be used for the advantage
            # get mean for each action
            values = self.target_value_model(latents).mean
            # V(z_t+1:T)     p(v_t+1|z_t+1)
            #next_value_target = self.target_value_model(latents[1:]).mean
            next_value_target = values[1:]
            # V(z_t:T-1) = r_t+1:T + nt_t+1:T γ V(z_t+1:T)
            target_values = rewards[1:] + (nonterminals[1:] * self.args.gamma * next_value_target.detach())
        critic_loss = self._value_loss(value_dist, target_values.detach())

        # == Actor π(a_t:T-1|z_t:T-1)
        # Get advantage A(z_t:T-1,a_t:T-1) = Q(z_t,a_t) - V(z_t) = E[r_t+1 +  γV(z_t+1)] - V(z_t) = r_t+1 + γV(z_t+1) - V(z_t)
        #advantage = (target_values - value_dist.mean).detach()   # (seq len, batch size, 1)
        advantage = (target_values - values[:-1]).detach()   # (seq len, batch size, 1)
        # Get policy the agent would follow if it encountered the latents
        actions, action_dist = self.policy_model(latents[:-1].detach()) #note: detached
        actor_loss = self._policy_loss(action_dist, actions, advantage)

        return actor_loss, critic_loss

    def _value_loss(self, dist, target_values):
        # NOTE: IN DV2 value_loss = -(weight.detach() * v_dist.log_prob(v_lambda.detach()).unsqueeze(-1)).sum(dim=0).mean()
        value_loss = -torch.mean(dist.log_prob(target_values))
        self.metrics['value_loss'] += value_loss.detach().item()
        self.metrics['mean_value'] += dist.mean.detach().mean().item()
        return self.args.value_weight * value_loss

    def _policy_loss(self, dist, actions, advantage):
        # note: actions detached
        #note in dv2 policy_loss = -(weight * objective).sum(dim=0).mean()
        actor_loss = torch.mean(-dist.log_prob(actions.round().detach()).unsqueeze(-1) * advantage)   # -log π(a|z) A(s,a)
        self.metrics['actor_loss'] += actor_loss.detach().item()
        self.metrics['policy_entropy'] += dist.entropy().detach().mean().item()
        return self.args.actor_weight * actor_loss
